    ...
    try:
        for gw in range(args.start_gw, args.end_gw + 1):
            ...
            with open(gw_standings_file, mode='w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(standings_headers[1:])
                for idx, team in enumerate(sorted_teams):
                    ...
                    standings_writer.writerow([
                        gw, idx + 1, team.get("Name"), team.get("PlayerName"), gw_pts, hit, net_gw_pts,
                        ld.get("SeasonTotalPoints"), ld.get("OverallRank"), ld.get("ActiveChip") or "None",
                        ld.get("Transfers", 0), team.get("TeamValue", 0.0), team.get("BankValue", 0.0)
                    ])
            ...
            with open(gw_lineups_file, mode='w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(lineups_headers[1:])
                for team in team_datas:
                    ...
                    for p in players:
                        ...
                        if p.get("IsSub", False) and not p.get("IsSubIn", False):
                            is_starting = False
                        writer.writerow([
                            manager, t_name, p.get("PlayerWebName"), p.get("TeamName"), pos,
                            p.get("Points"), p.get("IsCaptain", False), p.get("IsViceCaptain", False),
                            is_starting, p.get("IsSubIn", False), p.get("IsSubOut", False)
                        ])
                        lineups_writer.writerow([
                            gw, manager, t_name, p.get("PlayerWebName"), p.get("TeamName"), pos,
                            p.get("Points"), p.get("IsCaptain", False), p.get("IsViceCaptain", False),
                            is_starting, p.get("IsSubIn", False), p.get("IsSubOut", False)
                        ])
