...
def compile_data():
    ...
    with open(lineups_csv, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if gw not in gameweeks:
                # Standings should have created it, but guard just in case
                gameweeks[gw] = {
                    "standings": [],
                    "lineups": {}
                }
    ...
    # Convert gameweeks keys to string keys for standard JSON serialization
    json_data = {
...
def start_server():
    ...
    with socketserver.TCPServer(("", PORT), handler) as httpd:
        ...
        def open_browser():
            time.sleep(0.8) # Wait slightly for the server to be fully active
